= Purpose of Event Tables

*what the Event Table is*, *why you are building it*, and *how it helps identify and document the
two Krückeberg immigrants from Schaumburg-Lippe*.

== ✅ What the Event Table Is

The *Event Table* is a structured list of *all genealogically relevant
events* found in the Schaumburg-Lippe Lutheran church records,
including:

* *Baptisms*
* *Confirmations*
* *Marriages*
* *Deaths/Burials*
* *Communicant lists (if needed)*
* *Residence indicators*
* *House numbers (when they begin appearing)*
* *Occupations (when given)*
* *Patronymic/sponsor relationships*

Each row corresponds to *one recorded event*, not one person.

== ✅ Why the Event Table Matters for Your Case

You have *two Krückeberg immigrants*:

=== 1. *Johann Heinrich Krückeberg*

— emigrated *1849* to Adams County, Indiana

=== 2. *Carl Heinrich Krückeberg*

— emigrated *1850* to Adams County, Indiana

The Schaumburg-Lippe parish registers (Petzen, Frille, nearby) contain
*multiple Krückeberg families*, often:

* with the same given names
* living in the *same village* (Berenbusch, Petzen, etc.)
* without house numbers in early records
* without occupations in early records

So identities easily get confused.

The *Event Table solves this* by letting you:


* ✅ cluster events belonging to the same nuclear family 
* ✅ distinguish same-name individuals 
* ✅ track movement between villages 
* ✅ identify fathers and mothers even when not named 
* ✅ document all claims with proper citations 
* ✅ build *a defensible lineage* meeting BCG standards

== ✅ What Columns the Event Table Should Contain

This is the structure we previously established:

....
| Event Type (B, C, M, D)
| Event Date (exact or circa)
| Name of subject
| Father (as written; normalized in parentheses)
| Mother (as written; normalized in parentheses)
| Residence / village (as written)
| House number (when available)
| Occupation (if present)
| Sponsors / witnesses (as written)
| Notes (abbreviations, ambiguity, ditto marks)
| Siglum of record (your standardized form)
| Citation key / link to page
....

Optional advanced columns:

* ✅ inferred identity 
* ✅ confidence level 
* ✅ link to other event rows

== ✅ How the Event Table Connects to Building the Pedigree

For *each of the two immigrants*, the Event Table allows you to:

=== Step 1 — Identify the correct baptism record

(eliminating others with same name)

=== Step 2 — Confirm parents through:

* marriage entries
* siblings’ baptisms
* death records
* sponsor networks

=== Step 3 — Tie family to:

* a specific house number (later records)
* village cluster (Berenbusch, etc.)

=== Step 4 — Link to Indiana arrivals

* matching given names of children
* matching ages in US census
* matching Lutheran confirmation ages

=== Step 5 — Produce BCG-compliant proof

* negative evidence documented
* conflicting evidence resolved
* citations for every assertion

'''''

== ✅ Why This Helps Your German Researcher

The Event Table becomes:

* ✅ the *research brief* 
* ✅ the *working hypothesis map* 
* ✅ the *evidence
correlation tool*

So instead of saying:

____
“Please help me figure out which Krückeberg belongs to which family…”
____

You will say:

____
“Rows 22–41 appear to be the family of the 1849 emigrant, but rows 33
and 36 conflict due to residence wording. Please verify in archival
material A12–K2.”
____

That saves you *billable hours* and eliminates wheel-spinning.

'''''

== ✅ Where It Belongs in Your Antora Structure

This should live in the *research-context* component, in a module named:

....
modules/event-tables/
....

With pages such as:

....
krueckeberg-event-table-1700-1749.adoc
krueckeberg-event-table-1750-1799.adoc
krueckeberg-event-table-1800-1849.adoc
....

Each page will contain:

* ✅ the table 
* ✅ notation system 
* ✅ interpretation notes 
* ✅ links to record pages 
* ✅ developing hypotheses

== ✅ If you want next steps, tell me ONE thing:

=== Which parish should the first event table cover?

Choose one:

[arabic]
. *Petzen*
. *Frille*
. *Both together*
. *Whichever has the earliest Krückeberg entries*
. *Whichever is relevant to the 1849 emigrant*
. *Whichever is relevant to the 1850 emigrant*

Depending on your choice, I will produce:

* ✅ the table skeleton 
* ✅ recommended file names 
* ✅ siglum format 
* ✅ citation anchors 
* ✅ sponsor-network markers 
* ✅ Antora page template
ready to paste

Just reply with the number (1–6).
